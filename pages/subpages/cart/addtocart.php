<?php
$pageTitle = "AddCart"; // Set the page title
...
if (isset($_SESSION['valid_user']) && isset($_GET['productID']) && isset($_GET['price']) && isset($_GET['size'])) {
    ...
    $checkQuery = "SELECT Quantity FROM Cart WHERE Name = ? AND ProductID = ? AND Size = ?";
    ...
    if ($checkStmt->num_rows > 0) {
        ...
        $updateQuery = "UPDATE Cart SET Quantity = ?, Price = ? WHERE Name = ? AND ProductID = ? AND Size = ?";
        ...
        $addQuery = "INSERT INTO Cart (Name, ProductID, Quantity, Size, Price) VALUES (?, ?, 1, ?, ?)";
